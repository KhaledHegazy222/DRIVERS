...
static void send(u8 msg){
	...
	GPIO_SetPinVal(lcd_ctr,EN,1); // Enable = 1
	_delay_ms(1);
	GPIO_SetPinVal(lcd_ctr,EN,0); // Enable = 0
...
void LCD_Init(void){
	...
	GPIO_SetPinDir(lcd_ctr,EN,OUTPUT);
...
void LCD_Init4(void){
	...
	GPIO_SetPinVal(lcd_ctr,EN,1); 
	_delay_ms(1);
	GPIO_SetPinVal(lcd_ctr,EN,0); 
	_delay_ms(1);
	/* Init 4 Bit Mod */
...
u8 LCD_GetAddress(void){
	...
	GPIO_SetPinVal(lcd_ctr,EN,1); // Enable = 1
	_delay_ms(1);
	GPIO_SetPinVal(lcd_ctr,EN,2); // Enable = 0
